...
class MarketGame:
    ...
    def closing_price(self):
        try:
            ...
            return float(self.sample_data_hidden['Close'][len(self.sample_data_hidden) - 1])
    ...
    def balances(self):
        ...
        try:
            bh_equity = round((10000 / self.sample_data_hidden["Close"][503]) * float(
                self.sample_data_hidden["Close"][self.days_elapsed - 1]), 2)
        except KeyError:
            bh_equity = round((10000 / self.sample_data_hidden["Close"][503]) * float(
                self.sample_data_hidden["Close"][len(self.sample_data_hidden) - 1]), 2)
...
if __name__ == "__main__":
    ...
    def reconfigure_balance(days_delta=0):
        mg.latest_graph(days_delta)
        bal = mg.balances()
        bal_label.config(text=bal)

    # Display chart
    button_px = 3
    button_py = 3
    button = tk.Button(root, text='View Chart', command=mg.show_chart)
    button_1d = tk.Button(root, text='+1 day', command=lambda: reconfigure_balance(1))
    button_5d = tk.Button(root, text='+5 days', command=lambda: reconfigure_balance(5))
    button_20d = tk.Button(root, text='+20 days', command=lambda: reconfigure_balance(20))
    button_max = tk.Button(root, text='Skip to final', command=lambda: reconfigure_balance(252))
    ...
    # Update Cash & Holdings
    def reconfigure_bal_buy():
        mg.buy(entry_shares.get())
        balance = mg.balances()
        bal_label.config(text=balance)

    def reconfigure_bal_sell():
    ...
    button_buy = tk.Button(root, text='Buy Shares', command=reconfigure_bal_buy)
    button_sell = tk.Button(root, text='Sell Shares', command=reconfigure_bal_sell)
    ...
    # Cash & Holdings
    main_balance = mg.balances()
    bal_label = tk.Label(root, text=main_balance)
